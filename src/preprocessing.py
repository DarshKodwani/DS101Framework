...
def format_ocean_proximity(raw_df):
    """
    Changes the feature `ocean_proximity` to be lowercase and change the value `<1H OCEAN` to `within_one_hour_ocean`
    Args:
    ...
    """
    raw_df["ocean_proximity"] = ["within_one_hour_ocean" if val == "<1H OCEAN" else val for val in raw_df["ocean_proximity"]]
